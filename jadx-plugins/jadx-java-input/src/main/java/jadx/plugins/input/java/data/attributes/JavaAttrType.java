...
public final class JavaAttrType<T extends IJavaAttribute> {
	...
	private static final Map<String, JavaAttrType<?>> nameToType;
	...
	private static <A extends IJavaAttribute> JavaAttrType<A> bind(String name, IJavaAttributeReader reader) {
		JavaAttrType<A> attrType = new JavaAttrType<>(nameToType.size(), name, reader);
		nameToType.put(name, attrType);
		return attrType;
	}
